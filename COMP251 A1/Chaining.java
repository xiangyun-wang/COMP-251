...
public class Chaining {
    ...
    public int insertKey(int key){
        // get the ArrayList and add the key to the ArrayList

        // the return value is the collision number, which is the size-1
        // after adding the new key to the table

        ArrayList<Integer> tmp = this.Table.get(chain(key));
        tmp.add(key);
        return tmp.size()-1;
